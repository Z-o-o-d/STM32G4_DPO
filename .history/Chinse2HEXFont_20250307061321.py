def read_and_process_c_file(input_file_path, output_file_path, group_size):
    ...
    # 按指定组大小拼接16位数据
    combined_values = []
    for i in range(0, len(sixteen_bit_values), group_size):
        group = sixteen_bit_values[i: i + group_size]
        if len(group) == group_size:
            combined = 0
            for val in group:
                combined = (combined << 16) | val
            combined_values.append(combined)

    # 将结果转换为十六进制字符串列表
    hex_strings = [f"0x{value:0{4 * group_size}X}" for value in combined_values]
...
group_size = 3  # 可根据需要设置分组大小，例如 3 个或 4 个一组
