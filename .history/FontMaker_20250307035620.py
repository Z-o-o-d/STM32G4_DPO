...
size_menu = tk.OptionMenu(root, size_var, *font_sizes, command=lambda _: preview_font())
size_menu.pack(side=tk.LEFT, padx=10)

# 加粗和倾斜复选框
bold_var = tk.IntVar()
italic_var = tk.IntVar()
bold_checkbox = tk.Checkbutton(root, text="加粗", variable=bold_var, command=preview_font)
italic_checkbox = tk.Checkbutton(root, text="倾斜", variable=italic_var, command=preview_font)
...
for sample_size in sample_sizes:
    sample_label = tk.Label(root, text=f"字体 {sample_size} 号字预览", font=("Arial", sample_size))
    sample_label.pack(pady=5)
    sample_labels.append(sample_label)

# 固定窗口大小
root.geometry("600x700")  # 可根据实际情况调整宽度和高度
